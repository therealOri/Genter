...
def show_pass():
    cwd = os.getcwd()
    clear()
    cmd = f"notepad.exe {cwd}\pass.txt"
    os.system(cmd)
...
def change_creds(old_master_key, new_master_key):
    ...
    if len(D_old_key) and len(E_new_key) < 32 or len(D_old_key) and len(E_new_key) > 32:
        ...
        ids = c.fetchall()
        ldb = str(ids).replace("(", "").replace(",)", "").replace("'", "")
        ids_lst = ldb.strip('][').split(', ')
        ...
        ldta = str(enc_data_logins).replace("(", "").replace(",)", "").replace("'", "")
        plist = ldta.strip('][').split(', ')
                ...
                if sub_options[1] in sub_option:
                    if os.path.isfile(f'pwords{project_ext}'):
                        if os.path.isfile(f'pwords{project_ext}.oCrypted'):
                            ...
                            clear()
                            rmv_key = beaupy.prompt("Encryption key: ")
                            try:
                ...
                if sub_options[2] in sub_option:
                    if os.path.isfile(f'pwords{project_ext}'):
                        if os.path.isfile(f'pwords{project_ext}.oCrypted'):
                            ...
                            clear()
                            show_logins_key = beaupy.prompt("Encryption key: ")
                            try:
